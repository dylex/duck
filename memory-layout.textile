...
# If there is only one non-nullary constructor, the tag word is omitted from the boxed representation.
# If there is only one non-nullary constructor, no nullary constructors, and the single constructor has only one nonzero-size argument, that argument is stored unboxed.
...
Additional notes:
# The zero size argument removal rules mean that many types that should be isomorphic in memory actually are.  For example, @Maybe Int@ and @Either () Int@ have identical representations.
